...
def Positioning_relationship(tagfile, positionfile, spename):
...
if __name__ == '__main__':
    ...
    (same_strand_value, oppo_strand_value) = Positioning_relationship(args[0], args[1], options.spename)
    for i in range(2001):
        same_strand_value[i] = str(same_strand_value[i])
        oppo_strand_value[i] = str(oppo_strand_value[i])
    ofile = open(options.output,'w')
    ofile.write('Same_strand' + ' <- c(' + ', '.join(same_strand_value) + ')')
    ofile.write('Oppo_strand' + ' <- c(' + ', '.join(oppo_strand_value) + ')')
    ofile.close()    
